...
class _Class<T> with EqualsAndHashCode implements Class<T> {
  ...
  _Class(this._name, this._pd, this._package) {
  ...
  _Class.fromQualifiedName(this._name, this._pd) : _package = null {
  ...
  _Class.declared(this._declaration, this._pd) : 
  ...
  String getSignature() {
    ...
    if (_declaration is ClassDeclaration) {
      final typeParams = _declaration.getTypeArguments().isNotEmpty
          ? '<${_declaration.getTypeArguments().join(', ')}>'
          : '';
  ...
  List<Field> getEnumValues() {
    ...
    if (enumType != null) {
      return enumType.getValues().map((f) => Field.declared(f, enumType, _pd)).toList();
    }
