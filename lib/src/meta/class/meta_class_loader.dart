...
/// 
/// ## Cache Architecture
/// The _MetaClassLoader employs a multi-tier caching strategy:
/// 
/// ### Primary Caches
...
/// final loader = _MetaClassLoader();
/// 
/// // Load classes with caching
/// final stringClass = await loader.loadClass<String>('dart:core.String');
/// final listClass = await loader.loadClass<List>('dart:core.List');
/// 
...
/// 
/// {@endtemplate}
class _MetaClassLoader extends ClassLoader {
  // Statistics tracking
  int _hitCount = 0;
  int _missCount = 0;
  ...
  Class<T>? findClass<T>(String className, [ProtectionDomain? domain]) {
    ...
    try {
      ...
      // Log error and return null for not found
      return null;
    }
  }
  ...
  String _buildCacheKey(Class c) {
    ...
    final args = c.getDeclaration().getTypeArguments();
  ...
  List<Class> findTypeParameters(Class parentClass) {
    ...
    // Compute classes
    final args = parentClass.getDeclaration().getTypeArguments();
  ...
  List<Class> _computeAllInterfaceArguments(Class parentClass, bool declared) {
    if(declared) {
      return parentClass.getDeclaration().getInterfaces()
  ...
  List<Class> _computeAllMixinArguments(Class parentClass, bool declared) {
    if(declared) {
      return parentClass.getDeclaration().getMixins()
        .flatMap((i) => i.getTypeArguments())
        .map((i) => _getFromLink(i, parentClass.getProtectionDomain(), useType: true))
        .toList();
    } else {
  ...
  Class? findSuperClass(Class parentClass, [bool declared = true]) {
    ...
    // Compute classes
    final link = parentClass.getDeclaration().asClass()?.getSuperClass();
  ...
  List<Class> findSuperClassArguments(Class parentClass, [bool declared = true]) {
    ...
    final superType = parentClass.getDeclaration().asClass()?.getSuperClass();
  ...
  Type? extractComponentType(Class parentClass) {
    ...
    final typeArgs = parentClass.getDeclaration().getTypeArguments();
...
/// {@macro meta_class_loader}
final _MetaClassLoader MetaClassLoader = _MetaClassLoader();