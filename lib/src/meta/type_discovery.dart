...
class TypeDiscovery {
  ...
  static TypeDeclaration? findGeneric(String typeString) {
   final parseResult = GenericTypeParser.resolveGenericType(typeString);
    ...
    final baseDeclaration = findByName(baseName);
  ...
  static bool _isClassMirror(String string) => string == "_ClassMirror";
  ...
  /// ```
  /// {@endtemplate}
  static TypeDeclaration? findByType(Type type) {
    // Check cache first
    ...
    if(GenericTypeParser.isGeneric(type.toString())) {
      result ??= findGeneric(type.toString());
    }

    // Method 1: Direct runtime type matching in special types
    result ??= Runtime.getSpecialTypes().firstWhereOrNull((d) => _isClassMirror(d.getType().toString()) 
      ? d.getName().toString() == type.toString()
      : d.getType() == type
    );

    // Method 2: Search in classes
    result ??= _searchInClasses(type);

    // Method 3: Search in enums
    result ??= _searchInEnums(type);

    // Method 4: Search in mixins
    result ??= _searchInMixins(type);

    // Method 5: Search in typedefs
    result ??= _searchInTypedefs(type);

    // Method 6: Search in records
    result ??= _searchInRecords(type);
  ...
  static ClassDeclaration? _searchInClasses(Type type) {
    return Runtime.getAllClasses().firstWhereOrNull((d) => _isClassMirror(d.getType().toString()) 
      ? d.getName().toString() == type.toString()
      : d.getType() == type
    );
  }
  ...
  static EnumDeclaration? _searchInEnums(Type type) {
    return Runtime.getAllEnums().firstWhereOrNull((d) => _isClassMirror(d.getType().toString()) 
      ? d.getName().toString() == type.toString()
      : d.getType() == type
    );
  }
  ...
  static MixinDeclaration? _searchInMixins(Type type) {
    return Runtime.getAllMixins().firstWhereOrNull((d) => _isClassMirror(d.getType().toString()) 
      ? d.getName().toString() == type.toString()
      : d.getType() == type
    );
  }
  ...
  static TypedefDeclaration? _searchInTypedefs(Type type) {
    return Runtime.getAllTypedefs().firstWhereOrNull((d) => _isClassMirror(d.getType().toString()) 
      ? d.getName().toString() == type.toString()
      : d.getType() == type
    );
  }
  ...
  static RecordDeclaration? _searchInRecords(Type type) {
    return Runtime.getAllRecords().firstWhereOrNull((d) => _isClassMirror(d.getType().toString()) 
      ? d.getName().toString() == type.toString()
      : d.getType() == type
    );
  }
  ...
  /// ```
  /// {@endtemplate}
  static TypeDeclaration? findByName(String name) {
    // Check cache first
    ...
    // Method 2: Search in classes
    result ??= _findClassDeclarationByString(name);

    // Method 3: Search in enums
    result ??= _findEnumDeclarationByString(name);

    // Method 4: Search in mixins
    result ??= _findMixinDeclarationByString(name);

    // Method 5: Search in typedefs
    result ??= _findTypedefDeclarationByString(name);

    // Method 6: Search in records
    result ??= _findRecordDeclarationByString(name);

    // Search by type string representation
    result ??= _searchByTypeString(name);
  ...
  static ClassDeclaration? _findClassDeclarationByString(String name) {
    return Runtime.getAllClasses().firstWhereOrNull((d) => d.getName().toString() == name);
  }

  /// Find enum declaration by string name (simple or qualified)
  static EnumDeclaration? _findEnumDeclarationByString(String name) {
    return Runtime.getAllEnums().firstWhereOrNull((d) => d.getName().toString() == name);
  }

  /// Find mixin declaration by string name (simple or qualified)
  static MixinDeclaration? _findMixinDeclarationByString(String name) {
    return Runtime.getAllMixins().firstWhereOrNull((d) => d.getName().toString() == name);
  }

  /// Find typedef declaration by string name (simple or qualified)
  static TypedefDeclaration? _findTypedefDeclarationByString(String name) {
    return Runtime.getAllTypedefs().firstWhereOrNull((d) => d.getName().toString() == name);
  }

  /// Find record declaration by string name (simple or qualified)
  static RecordDeclaration? _findRecordDeclarationByString(String name) {
    return Runtime.getAllRecords().firstWhereOrNull((d) => d.getName().toString() == name);
  }

  /// Search by type string representation
  static TypeDeclaration? _searchByTypeString(String typeString) {
    return Runtime.getAllTypes().firstWhereOrNull((d) => d.getName().toString() == typeString);
  }
