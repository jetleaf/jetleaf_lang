...
// Cache the class lookup map at the instance level
Map<String, ClassDeclaration>? _classLookupCache;
...
class DefaultClassLoader extends ClassLoader {
  ...
  Map<String, ClassDeclaration> _getClassLookupMap() {
    if (_classLookupCache == null) {
      final allClasses = Runtime.getAllClasses();
      _classLookupCache = {
        for (var clazz in allClasses)
          clazz.getQualifiedName(): clazz,
      };
    }
    return _classLookupCache!;
  }

  List<Class> _computeSubclasses(Class parentClass) {
    final classMap = _getClassLookupMap(); // <-- Get once
    final subClasses = <Class>[];
    final pd = parentClass.getProtectionDomain();
    
    for (ClassDeclaration clazz in classMap.values) {
      if (_isSubtypeOf(clazz, parentClass, classMap)) { // <-- Pass the map
        Class classToAdd;
    ...
    return subClasses; // <-- Remove redundant toSet().toList()
  }

  bool _isSubtypeOf(ClassDeclaration clazz, Class parentClass, Map<String, ClassDeclaration> classMap) {
    final visited = <String>{};
    return _isSubtypeOfRecursive(clazz, parentClass, visited, classMap);
  }

  bool _isSubtypeOfRecursive(
    ClassDeclaration clazz, 
    Class parentClass, 
    Set<String> visited,
    Map<String, ClassDeclaration> classMap, // <-- Pass the map down
  ) {
    final clazzQualifiedName = clazz.getQualifiedName();
    if (visited.contains(clazzQualifiedName)) {
      return false;
    }
    visited.add(clazzQualifiedName);

    // Check direct superclass
    LinkDeclaration? supertype = clazz.getSuperClass();
    if (supertype != null && _matchesParent(supertype, parentClass)) {
      return true;
    }

    // Check direct interfaces
    for (LinkDeclaration interface in clazz.getInterfaces()) {
      if (_matchesParent(interface, parentClass)) {
        return true;
      }
    }
    ...
    if (supertype != null) {
      ClassDeclaration? superClassDecl = _getClassDeclarationFast(supertype, classMap); // <-- O(1) lookup
      if (superClassDecl != null && _isSubtypeOfRecursive(superClassDecl, parentClass, visited, classMap)) {
        return true;
      }
    }
    ...
    for (LinkDeclaration interface in clazz.getInterfaces()) {
      ClassDeclaration? interfaceDecl = _getClassDeclarationFast(interface, classMap); // <-- O(1) lookup
      if (interfaceDecl != null && _isSubtypeOfRecursive(interfaceDecl, parentClass, visited, classMap)) {
        return true;
      }
    }
  ...
  ClassDeclaration? _getClassDeclarationFast(LinkDeclaration link, Map<String, ClassDeclaration> classMap) {
    // Try qualified name first (most reliable)
    final qName = link.getPointerQualifiedName();
    ClassDeclaration? result = classMap[qName];
    if (result != null) return result;
    
    // Fallback: search by type (only if needed)
    final targetType = link.getPointerType();
    for (var clazz in classMap.values) {
      if (clazz.getType() == targetType) {
        return clazz;
      }
    }
  ...
  // /// Helper to check if a link declaration matches the parent class.
  bool _matchesParent(LinkDeclaration link, Class parentClass) {
    return link.getPointerType() == parentClass.getType() ||
          link.getType() == parentClass.getType() ||
          link.getPointerQualifiedName() == parentClass.getQualifiedName();
  }
